...
# 文件路径
filename1 = r'D:\AntiUAV\dataset_infrared\SPAD\2024-10-30_21-41-52_Delay-0_Width-200.raw'
...
def raw2n3(filename):
    ...
    #     raw_data = f.read()
    #     f.seek(offset_byte)  # 找到设定的偏移位置
    #     binary_data = f.read(data_size)  # 读取指定数量的二进制数据
    ...
    # print(raw_data)
    data_gpu = raw_data.reshape((480, 4096, 10))
    # print(data_gpu)
    data_gpu = np.transpose(data_gpu, (0,2,1))
    data_gpu = data_gpu.reshape(4800, 64,64)
    # data_gpu = data_gpu[:480,:,:]
...
def plot_pc(pc):
    ...
    pc = ax.scatter(pc[:, 1], pc[:, 2], pc[:, 0], c=normalized_strength, s=3, cmap='jet')
    # 设置坐标轴
    ax.set_xlabel('Y')
    ax.set_ylabel('Z')
    ax.set_zlabel('X')
