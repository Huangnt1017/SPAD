...
import spikingjelly1.cext.functional
import warnings
import math
import numpy as np
import time
    ...
    * :ref:`中文API <SparseLinear-cn>`
    ...
    def forward(self, sparse: torch.Tensor) -> torch.Tensor:
        if self.bias is None:
            return spikingjelly1.cext.functional.sparse_mm_dense(sparse, self.weight.t())
        else:
            return spikingjelly1.cext.functional.sparse_mm_dense(sparse, self.weight.t()) + self.bias
...
class AutoSparseLinear(nn.Linear):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if sparsity < csp:
            ...
            if self.bias is None:
                return spikingjelly1.cext.functional.sparse_mm_dense(x, self.weight)
            else:
                return spikingjelly1.cext.functional.sparse_mm_dense(x, self.weight) + self.bias    
