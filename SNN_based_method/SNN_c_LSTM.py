...
设计说明:
    ...
    输出协议不变，只把脉冲层替换成显式的时空 ConvLSTM 递推。这样能够在
    不改训练脚本和 loss 接口的前提下，对比脉冲递推与 LSTM 递推的差异。
"""
...
class ConvLSTMCell(nn.Module):
    """标准 ConvLSTM cell，输入和隐藏状态均保留空间布局。"""
    ...
    def _init_state(
        ...
        """根据当前输入尺寸创建零初始化的 ``(h_t, c_t)``。"""
        batch_size, _, height, width = x_t.shape
        h_t = x_t.new_zeros(batch_size, self.hidden_channels, height, width)
        c_t = x_t.new_zeros(batch_size, self.hidden_channels, height, width)
        return h_t, c_t
    ...
    def forward(
        ...
        h_t, c_t = self._init_state(x_seq[0]) if state is None else state
...
class _StemLSTM(nn.Module):
    """Stem 网络的 ConvLSTM 版本。"""
    ...
    def __init__(
        ...
        self.lstm = ConvLSTMCell(c_hidden, c_hidden, kernel_size=3)
    ...
    def forward(
        ...
        x_seq, state = self.lstm(x_seq, state)
        x_seq = _seq_to_ann_forward(x_seq, self.conv2, self.bn2)
        return x_seq, state


class LSTMBlock(nn.Module):
    """保持残差结构的 ConvLSTM 版时序块。"""

    def __init__(self, c_hidden: int, spike_backend: str = "auto") -> None:
        super().__init__()
        self.spike_backend = str(spike_backend).lower()
        self.lstm_in = ConvLSTMCell(c_hidden, c_hidden, kernel_size=3)
        self.ms_dsconv = MultiScaleDSConv(c_hidden)
        self.lstm_mid = ConvLSTMCell(c_hidden, c_hidden, kernel_size=3)
...
class _GateHeadLSTM(nn.Module):
    ...
    def __init__(self, c_hidden: int, spike_backend: str = "auto") -> None:
        ...
        self.lstm1 = ConvLSTMCell(c_hidden, c_hidden, kernel_size=3)
        self.conv1 = nn.Conv2d(c_hidden, self.mid_channels, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(self.mid_channels)
        self.lstm2 = ConvLSTMCell(self.mid_channels, self.mid_channels, kernel_size=3)
    ...
    def forward(
        ...
        x_seq, state1 = self.lstm1(x_seq, state1)
        x_seq = _seq_to_ann_forward(x_seq, self.conv1, self.bn1)
        x_seq, state2 = self.lstm2(x_seq, state2)
        x_seq = _seq_to_ann_forward(x_seq, self.conv2)
        return torch.sigmoid(x_seq), (state1, state2)


class SNN_c_LSTM(nn.Module):
    """保持 SNN 输出协议不变的 ConvLSTM 成像模型。"""
    ...
    def __init__(
        ...
        self.stem = _StemLSTM(c_enc, self.C, spike_backend=self.spike_backend)
        self.blocks = nn.ModuleList(
            [LSTMBlock(self.C, spike_backend=self.spike_backend) for _ in range(num_blocks)]
        )
        self.gate_head = _GateHeadLSTM(self.C, spike_backend=self.spike_backend)
