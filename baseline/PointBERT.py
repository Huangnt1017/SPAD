...
GitHub:  https://github.com/lulutang0608/Point-BERT
Local:   D:\essay\3d目标检测复现仓库\Point-BERT-master

完整复现 Point-BERT 分类模型:
- Group: FPS + KNN 分组构建局部 patches (与 dVAE 共享)
- Encoder: Mini-PointNet 编码 patches → patch tokens
- TransformerEncoder: 标准 Transformer (含 cls_token) + 位置编码
- 分类头: cls_token + 全局最大池化拼接 → MLP
...
# ============================================================================
# Group: FPS + KNN 分组 (Point-BERT dVAE 的 Group 模块)
# ============================================================================
...
# ============================================================================
# Encoder: Mini-PointNet 编码器 (与 dVAE 共享)
# ============================================================================
...
class TransformerEncoder(nn.Module):
    """Transformer 编码器: 堆叠 Block + 位置编码相加。

    与 Point-BERT 官方实现一致:
    - 输入 tokens + pos embedding 相加后送入 blocks
    """
...
class PointBERTClassification(nn.Module):
    """Point-BERT 分类 + 3D BBox 模型 (适配 SPAD 训练管道)。

    架构:
    1. Group: FPS + KNN 分组构建局部 patches
    2. Encoder: Mini-PointNet 编码为 patch tokens
    3. Reduce dim: Linear(encoder_dims → trans_dim)
    4. cls_token + 位置编码
    5. TransformerEncoder: 标准 Transformer
    6. 分类头: cls_token + 全局最大池化拼接 → MLP
    ...
    def _normalize_input_points(x):
        """统一输入为 (B, N, 4) xyzi。"""
        if x.ndim != 3:
            raise ValueError(f"PointBERTClassification expects 3D input, got {tuple(x.shape)}")
        if x.shape[-1] in (3, 4):
            points = x
    ...
    def forward(self, x):
        ...
        neighborhood, center = self.group_divider(pts)  # (B, G, K, 3), (B, G, 3)
        ...
        # 4. 准备 cls_token + 位置编码
        cls_tokens = self.cls_token.expand(group_input_tokens.size(0), -1, -1)  # (B, 1, C)
        cls_pos = self.cls_pos.expand(group_input_tokens.size(0), -1, -1)       # (B, 1, C)
        pos = self.pos_embed(center)  # (B, G, C)

        # 拼接 cls_token 和 patch tokens
        ...
        cls_feat = x[:, 0]                                  # (B, C)
        max_feat = x[:, 1:].max(dim=1)[0]                   # (B, C)
        concat_f = torch.cat([cls_feat, max_feat], dim=-1)   # (B, 2C)
...
def _gpu_memory_test():
    ...
    for bs in [4, 8, 16, 32]:
        try:
            m = PointBERTClassification(num_classes=26).cuda()
...
def _quick_test():
    ...
    print(f"Testing Point-BERT on {device}")

    model = PointBERTClassification(num_classes=26).to(device)
    ...
    print("✓ Point-BERT works correctly")
